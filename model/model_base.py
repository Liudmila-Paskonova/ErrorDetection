...
class Code2VecModelBase(abc.ABC):
    ...
    def _write_code_vectors(self, file, original_names, code_vectors):
        for i in range(code_vectors.shape[0]):
            file.write(','.join(original_names[i].split('|')))
            file.write(',')
            file.write(','.join(map(str, code_vectors[i])) + '\n')

    def _write_original_names(self, file, original_names):
        for vec in original_names:
            file.write(vec + '\n')
