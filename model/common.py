...
'''
Here there're several helpful functions, mostly for data processing
'''
...
class common:
    ...
    # histogram is file <token/tokenID, number of these tokens>
    @staticmethod
    ...
    # get the vocabulary with words (with restrictions on MAX_TOKEN_VOCAB_SIZE and others)
    @staticmethod
    def load_vocab_from_histogram(path, min_count=0, start_from=0, max_size=None, return_counts=False):
        if max_size is not None:
            word_to_index, index_to_word, next_index, word_to_count = \
                common._load_vocab_from_histogram(
            ...
            min_count = sorted(word_to_count.values(),
                               reverse=True)[max_size] + 1
    ...
    def save_word2vec_file(output_file, index_to_word, vocab_embedding_matrix: np.ndarray):
        ...
        for word_idx in range(0, vocab_size):
            ...
            output_file.write(
                ' '.join(map(str, vocab_embedding_matrix[word_idx])) + '\n')

    @staticmethod
    def calculate_max_contexts(file):
        contexts_per_word = common.process_test_input(file)
        return max(
            [max(l, default=0) for l in [[len(contexts)
                                          for contexts in prog.values()] for prog in contexts_per_word]],
            default=0)
    ...
    def legal_method_names_checker(special_words, name):
        return name != special_words.OOV and re.match(r'^[a-zA-Z|\-\d]+$', name)
    ...
    def parse_prediction_results(raw_prediction_results, unhash_dict, special_words, topk: int = 5) -> List['MethodPredictionResults']:
        ...
        for single_method_prediction in raw_prediction_results:
            current_method_prediction_results = MethodPredictionResults(
                single_method_prediction.original_name)
    ...
    def squeeze_single_batch_dimension_for_np_arrays(arrays):
        assert all(array is None or isinstance(array, np.ndarray)
                   or isinstance(array, tf.Tensor) for array in arrays)
        return tuple(
    ...
    def get_first_match_word_from_top_predictions(special_words, original_name, top_predicted_words) -> Optional[Tuple[int, str]]:
        ...
        for suggestion_idx, predicted_word in enumerate(common.filter_impossible_names(special_words, top_predicted_words)):
            normalized_possible_suggestion = common.normalize_word(
                predicted_word)
