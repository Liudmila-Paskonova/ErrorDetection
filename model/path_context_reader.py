...
class PathContextReader:
    ...
    def _create_dataset_pipeline(self, input_data_rows: Optional = None) -> tf.data.Dataset:
        ...
        # load data for next epoch for faster training
    ...
    def _filter_input_rows(self, *row_parts) -> tf.bool:
        ...
        if self.estimator_action.is_evaluate:
            ...
            cond = tf.logical_and(
                word_is_valid, any_contexts_is_valid)  # scalar
    ...
    def _map_raw_dataset_row_to_input_tensors(self, *row_parts) -> ReaderInputTensors:
        ...
        contexts_str = tf.stack(
            row_parts[1:(self.config.MAX_CONTEXTS + 1)], axis=0)
        split_contexts = tf.compat.v1.string_split(
            contexts_str, sep=',', skip_empty=False)
        ...
        path_source_token_indices = self.vocabs.token_vocab.lookup_index(
            path_source_token_strings)  # (max_contexts, )
        path_indices = self.vocabs.path_vocab.lookup_index(
            path_strings)  # (max_contexts, )
        path_target_token_indices = self.vocabs.token_vocab.lookup_index(
            path_target_token_strings)  # (max_contexts, )
        ...
        context_valid_mask = tf.cast(reduce(
            tf.logical_or, valid_word_mask_per_context_part), dtype=tf.float32)  # (max_contexts, )
