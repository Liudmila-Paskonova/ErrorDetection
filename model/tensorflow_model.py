...
# eager execution disable
# automation above TF computational map (=e g definition on neural network layes)
...
class Code2VecModel(Code2VecModelBase):
        ...
        with open('log.txt', 'w') as log_output_file:
            if self.config.EXPORT_CODE_VECTORS:
                code_vectors_file = open(
                    self.config.TEST_DATA_PATH + '.vectors', 'a')
            total_predictions = 0
            total_prediction_batches = 0
            # F1, precision, recall and so on
                ...
                while True:
                    ...
                    if self.config.EXPORT_CODE_VECTORS:
                        self._write_code_vectors(
                            code_vectors_file, original_names, code_vectors)
                      #  self._write_original_names(
                        #   original_names_file, original_names)
        ...
        with tf.compat.v1.variable_scope('model'):
            tokens_vocab = tf.compat.v1.get_variable(
                ...
                shape=(self.vocabs.token_vocab.size,
                       self.config.TOKEN_EMBEDDINGS_SIZE), dtype=tf.float32,
                # initializing vocab weights (specila algo)
            ...
            targets_vocab = tf.compat.v1.get_variable(
                ...
                shape=(self.vocabs.target_vocab.size,
                       self.config.TARGET_EMBEDDINGS_SIZE), dtype=tf.float32,
                initializer=tf.compat.v1.initializers.variance_scaling(scale=1.0, mode='fan_out', distribution="uniform"))
            ...
            paths_vocab = tf.compat.v1.get_variable(
                ...
                shape=(self.vocabs.path_vocab.size,
                       self.config.PATH_EMBEDDINGS_SIZE), dtype=tf.float32,
                initializer=tf.compat.v1.initializers.variance_scaling(scale=1.0, mode='fan_out', distribution="uniform"))
    ...
    def _calculate_weighted_contexts(self, tokens_vocab, paths_vocab, attention_param, source_input, path_input,
        ...
        #  embedding_lookup ==  returns the elements of params according to the indexes specified by ids
        # dim = 128
        # here we take $batch lines of tokens, targets' and path vocabs => 3D matrices!
        source_word_embed = tf.nn.embedding_lookup(
            params=tokens_vocab, ids=source_input)  # (batch, max_contexts, dim)
        ...
        target_word_embed = tf.nn.embedding_lookup(
            params=tokens_vocab, ids=target_input)  # (batch, max_contexts, dim)
        # c_i = [value_vocab[s], path_vocab[j], value_vocab[t]]
        ...
        # elem-wise natural log
        mask = tf.math.log(valid_mask)  # (batch, max_contexts)
        # just addinf +1 dim to shape
        mask = tf.expand_dims(mask, axis=2)  # (batch, max_contexts, 1)
        batched_contexts_weights += mask  # (batch, max_contexts, 1)
        # a_i
        attention_weights = tf.nn.softmax(
            batched_contexts_weights, axis=1)  # (batch, max_contexts, 1)

        batched_embed = tf.reshape(
            flat_embed, shape=[-1, self.config.MAX_CONTEXTS, self.config.CODE_VECTOR_SIZE])  # (batch, max_contexts, 384)
        # !!! SUM along all contexts! -> code vector v
        code_vectors = tf.reduce_sum(tf.multiply(
            batched_embed, attention_weights), axis=1)  # (batch, dim * 3)

        # Normalize
        code_vectors = tf.math.l2_normalize(code_vectors, axis=1)
